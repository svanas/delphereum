...
procedure getBalance(client: TWeb3; address: TAddress;
  callback: TASyncQuantity); overload;
procedure getBalance(client: TWeb3; address: TAddress; const block: string;
  callback: TASyncQuantity); overload;

procedure call(client: TWeb3; &to: TAddress; const func: string;
  args: array of const; callback: TASyncString); overload;
procedure call(client: TWeb3; from, &to: TAddress; const func: string;
  args: array of const; callback: TASyncString); overload;
procedure call(client: TWeb3; &to: TAddress; const func, block: string;
  args: array of const; callback: TASyncString); overload;
procedure call(client: TWeb3; from, &to: TAddress; const func, block: string;
  args: array of const; callback: TASyncString); overload;

procedure call(client: TWeb3; &to: TAddress; const func: string;
  args: array of const; callback: TAsyncTuple); overload;
procedure call(client: TWeb3; from, &to: TAddress; const func: string;
  args: array of const; callback: TAsyncTuple); overload;
procedure call(client: TWeb3; &to: TAddress; const func, block: string;
  args: array of const; callback: TAsyncTuple); overload;
...
function sign(privateKey: TPrivateKey; const msg: string;
  chain: TEthChain): string;
...
procedure getBalance(client: TWeb3; address: TAddress; const block: string;
  callback: TASyncQuantity);
begin
  web3.JSON.rpc.Send(client.URL, 'eth_getBalance', [address, block],
    procedure(resp: TJsonObject; err: Exception)
    begin
...
procedure call(client: TWeb3; &to: TAddress; const func: string;
args: array of const; callback: TASyncString);
begin
...
// https://github.com/ethereum/wiki/wiki/Ethereum-Contract-ABI#argument-encoding
  ...
  begin
    ...
    begin
      case arg.VType of
        vtInteger:
          Result := Result + web3.utils.fromHex
            ('0x' + IntToHex(arg.VInteger, 64));
        vtString:
          Result := Result + web3.utils.fromHex
            (toHex32(UnicodeString(PShortString(arg.VAnsiString)^)));
        vtWideString:
          Result := Result + web3.utils.fromHex
            (toHex32(WideString(arg.VWideString^)));
        vtInt64:
          Result := Result + web3.utils.fromHex
            ('0x' + IntToHex(arg.VInt64^, 64));
        vtUnicodeString:
          Result := Result + web3.utils.fromHex
            (toHex32(string(arg.VUnicodeString)));
      end;
    end;
  end;
...
begin
  ...
  obj := web3.JSON.Unmarshal(Format('{"from": %s, "to": %s, "data": %s}',
  ...
  try
    ...
    web3.JSON.rpc.Send(client.URL, 'eth_call', [obj, block],
      procedure(resp: TJsonObject; err: Exception)
      begin
        if Assigned(err) then
          callback('', err)
...
procedure call(client: TWeb3; &to: TAddress; const func: string;
args: array of const; callback: TAsyncTuple);
begin
  call(client, ADDRESS_NULL, &to, func, args, callback);
end;
...
var
  ...
  tup: TTuple;
begin
  call(client, from, &to, func, block, args,
    procedure(const hex: string; err: Exception)
    begin
      if Assigned(err) then
        callback([], err)
      else
...
var
  ...
  V: TBigInteger;
begin
  Params := web3.eth.crypto.PrivateKeyFromByteArray
    (fromHex(string(privateKey)));
  Signer := TECDsaSignerEx.Create
    (THMacDsaKCalculator.Create(TDigestUtilities.GetDigest('SHA-256')));
  try
    Signer.Init(True, Params);
    Signature := Signer.GenerateSignature
      (sha3(TEncoding.UTF8.GetBytes(#25 + 'Ethereum Signed Message:' + #10 +
      IntToStr(Length(msg)) + msg)));
...
//     else
    ...
    Result := toHex(Signature.r.ToByteArrayUnsigned +
      Signature.s.ToByteArrayUnsigned + V.ToByteArrayUnsigned);
  finally
    Signer.Free;
  end;
end;
